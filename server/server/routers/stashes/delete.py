...
async def delete_stash(name: str = 'my_stash_one'):
    async with await psycopg.AsyncConnection.connect(stashgres_client.kv) as aconn:
        async with aconn.cursor(row_factory=dict_row) as acur:
            ...
            _query_3 = f"DROP TABLE documents_{_del_stash_id} CASCADE;"
            _query_3_N = f"DROP TABLE nodes_{_del_stash_id} CASCADE;"
            _query_3_I = f"DROP TABLE images_{_del_stash_id} CASCADE;"
            _query_3_A = f"DROP TABLE audio_{_del_stash_id} CASCADE;"
            _query_4 = f"DROP TABLE filestore_{_del_stash_id} CASCADE;"
