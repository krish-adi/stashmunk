...
# async def add_knowledge(stash: Stash, file: UploadFile= None):
async def add_knowledge(stash: Stash):
    async with await psycopg.AsyncConnection.connect(stashgres_client.kv) as aconn:
        async with aconn.cursor(row_factory=dict_row) as acur:
            ...
            print(type(_parser_response))
            print(_parser_response)

            return {'client_id': _client_id, 'stash_id': _insert_stash_id}
