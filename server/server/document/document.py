...
class Document:
    ...
    def traverse_and_apply(self, func: Callable[[Node], None], direction: str = 'root', parallel: bool = False) -> None:
        ...
        if parallel:
            ...
            async def async_apply_helper(node_list, document, func):
                await asyncio.gather(*(func(**{'node': node, 'document': document}) for node in node_list))
            ...
            for _level in _range:
                asyncio.run(async_apply_helper(
    ...
    async def atraverse_and_apply(self, func: Callable[[Node], None], direction: str = 'root', parallel: bool = False) -> None:
        ...
        if parallel:
            ...
            if direction == 'root':
                ...
                async def traverse_and_apply_helper(document: Document, node: Node, func: Callable[[Node, List[Node]], None]) -> None:
                    func(**{'node': node, 'document': document})
                    for child_id in node.children:
                        child_node = document.find_node(child_id)
                        if child_node is not None:
                            await traverse_and_apply_helper(
                                document, child_node, func)
